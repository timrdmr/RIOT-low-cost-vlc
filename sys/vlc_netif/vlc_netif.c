...
#define DATARATE_BITS_PER_SECOND 33000
...
static int _netif_send(gnrc_netif_t *netif, gnrc_pktsnip_t *pkt)
{
    ...
    while (next_pkt) {
        ...
        DEBUG("add data of package with data size %i to send buffer\n", next_pkt->size);
...
static gnrc_pktsnip_t *_netif_recv(gnrc_netif_t *netif)
{
    ...
    // TODO: check why addresses cannot compared this way
                              ...
                              void *value, size_t max_len)
{
    ...
    switch (opt) {
        ...
        case NETOPT_DEVICE_TYPE:
            assert(max_len == sizeof(uint16_t));
            *((uint16_t *)value) = NETDEV_TYPE_VLC;    // TODO: not working TEST_SUITES not defined
            res = sizeof(uint16_t);
            DEBUG("NETOPT_DEVICE_TYPE\n");
            break;
        default:
            break;
    }
